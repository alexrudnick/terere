...
#        if os.path.exists(os.path.join(fst_directory, filename)):
        if explicit:
            ...
            return FST.restore_parse_from_files(explicit, name,
                                                ...
                                                verbose=verbose)
#            return FST.restore_parse(fst_directory, fst_file=None,
#                                     label=name,
#                                     cascade=cascade,
#                                     path=explicit[0],
#                                     weighting=weighting,
    ...
    def load(filename, cascade=None, weighting=None,
        ...
        if suffix == 'fst':
            ...
            mtax.parse(label, open(filename, encoding='utf-8').read(),
                       verbose=verbose)
            mtax.compile(gen=gen, verbose=verbose)
            return fst
            ...
            if verbose:
                print('Loading sublexicon from {}'.format(filename))
            treeified = treeify_file(filename, seg_units=seg_units,
