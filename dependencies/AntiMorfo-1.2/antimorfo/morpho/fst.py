...
import re, os, random, tempfile, copy, time, functools
from collections import deque
from subprocess import Popen, PIPE
# Required for weights. (MG)
...
# -> state
INIT_RE = re.compile(r'->\s*(\S+)$', re.U)
# state ->
FINAL_RE = re.compile(r'(\S+)\s*->\s*(?:\[([^\]]*)\])?$', re.U)
# src -> dest [arc] [weight]
ARC_RE = re.compile(r'(\S+)?\s*->\s*(\S+)\s*\[(.*?)\]\s*(.*?)$', re.U)
# src -> dest <arc> [weight]
MULT_ARC_RE = re.compile(r'(\S+)?\s*->\s*(\S+)\s*<([^:]*?)(:?)([^:]*?)>\s*(.*?)$', re.U)
# src -> dest >>casc<<
CASC_RE = re.compile(r'(\S+)?\s*->\s*(\S+)\s*>>(.*?)<<\s*(.*?)$', re.U)
# src -> dest >FST<
FST_RE = re.compile(r'(\S+)?\s*->\s*(\S+)\s*>(.*?)<\s*(.*?)$', re.U)
# src -> dest +lex+
LEX_DEST_RE = re.compile(r'(\S+)?\s*->\s*(\S+)\s*\+(.*?)\+\s*(.*?)$', re.U)
# src +lex+
LEX_RE = re.compile(r'(\S+)?\s*\+(.*?)\+\s*(.*?)$', re.U)
# cascade name = {0, 1, 3, ...}
...
######################################################################
#{ Finite State Transducer Cascade (added by MG)
######################################################################
...
#        print('Weight to parse', weight)
    ...
    @staticmethod
    def shelve(fst, filename=None, directory='', string_weights=True):
        """Shelve an FST to a file (MG)."""
        if not filename:
            filename = os.path.join(directory, fst.label + '.shf')
        if string_weights:
            FST.stringify_weights(fst)
        shelf = shelve.open(filename, writeback=True)
        # Write all state attributes
        print('Shelving states')
        shelf['initial_state'] = fst._initial_state
        shelf['is_final'] = fst._is_final
        shelf['state_descr'] = fst._state_descr
        shelf['finalizing_string'] = fst._finalizing_string
        print('Shelving arcs')
        shelf['incoming'] = fst._incoming
        shelf['outgoing'] = fst._outgoing
        shelf['src'] = fst._src
        shelf['dst'] = fst._dst
        shelf['in_string'] = fst._in_string
        shelf['out_string'] = fst._out_string
        shelf['arc_descr'] = fst._arc_descr
        shelf['n_arcs'] = fst._n_arcs
        print('Shelving weights')
        shelf['weighting'] = fst._weighting
        shelf['final_weight'] = fst._final_weight
        shelf['weight'] = fst._weight
        print('Shelving miscellaneous stuff')
        shelf['stringsets'] = fst._stringsets
        shelf['seg_units'] = fst.seg_units
        shelf['sigma'] = fst._sigma
        shelf['inserted'] = fst._inserted
        print('Closing shelf')
        shelf.close()

    @staticmethod
    def unshelve(fst_name, filename, weighting=UNIFICATION_SR, verbose=False):
        """Unshelve an FST shelved in a file (MG)."""
        # Choose a label & create the FST.
        shelf = shelve.open(filename)
        fst = FST(fst_name, weighting=weighting)
        # Write all state attributes
        if verbose:
            print('Unshelving states')
        fst._initial_state = shelf['initial_state']
        fst._is_final = shelf['is_final']
        fst._state_descr = shelf['state_descr']
        fst._finalizing_string = shelf['finalizing_string']
        if verbose:
            print('Unshelving arcs')
        fst._incoming = shelf['incoming']
        fst._outgoing = shelf['outgoing']
        fst._src = shelf['src']
        fst._dst = shelf['dst']
        fst._in_string = shelf['in_string']
        fst._out_string = shelf['out_string']
        fst._arc_descr = shelf['arc_descr']
        fst._n_arcs = shelf['n_arcs']
        if verbose:
            print('Unshelving weights')
        # Why doesn't this work??
        fst._final_weight = shelf['final_weight']
        fst._weight = shelf['weight']
        if verbose:
            print('Unshelving miscellaneous stuff')
        fst._stringsets = shelf['stringsets']
        fst._sigma = shelf['sigma']
        fst._inserted = shelf['inserted']
        fst.seg_units = shelf['seg_units']
        if verbose:
            print('Closing shelf')
        shelf.close()
        return fst
    ...
    @staticmethod
    def write(fst, filename=None, directory='', shelve=True, features=False, exclude_features=[]):
        """Write an FST to a file, either in FST text format or shelved (MG)."""
        if not filename:
            extension = '.fst'
            if shelve:
                extension = '.shf'
        ...
        if shelve:
            print('Shelving FST in', filename)
            FST.shelve(fst, filename)
        else:
            print('Writing FST to', filename)
            out = open(filename, 'w', encoding='utf-8')
            # Write the features and values if features is True
            if features:
    ...
    def restore(fst_name, directory='', cascade=None, weighting=UNIFICATION_SR, seg_units=[], empty=True,
                ...
                generate=False, simplified=False, create_weights=True, unshelve=True, verbose=False):
        ...
        if unshelve:
            # Look for shelved file
            filename = name + '.shf'
            path = os.path.join(directory, filename)
            if os.path.exists(path):
                if verbose:
                    print('Unshelving FST', name, 'from', filename)
                return FST.unshelve(name, path, weighting=weighting)
            # Not found, try empty FST even if not asked to look for it
            if not empty:
                filename = empty_name + '.shf'
                path = os.path.join(directory, filename)
                if os.path.exists(path):
                    if verbose:
                        print('Unshelving FST', empty_name, 'from', filename)
                    return FST.unshelve(empty_name, path)
        # In other cases we need the cascade
        if not cascade:
            path = os.path.join(directory, name + '.cas')
            if os.path.exists(path):
        ...
        if cascade != None:
            ...
            if os.path.exists(os.path.join(directory, filename)):
                ...
                return FST.restore_parse(directory, filename, cascade=cascade, weighting=weighting,
                                         seg_units=seg_units,
    ...
    def load(filename, cascade=None, weighting=None, seg_units=[], verbose=False, lex_features=False,
        ...
        if suffix == 'fst':
            ...
            return FST.parse(label, open(filename, encoding='utf-8').read(), weighting=weighting,
                             cascade=cascade, directory=directory, seg_units=seg_units, weight_constraint=weight_constraint,
                             verbose=verbose)
        elif suffix == 'lex':
            # It's a file in lexicon format; treeify the file, then convert the tree to an FST
            return FST.tree_to_fst(treeify_file(filename, seg_units=seg_units, features=lex_features, dest=dest_lex,
                                                verbose=verbose),
                                   label, cascade=cascade, weighting=weighting, lex_features=lex_features,
                                   weight_constraint=weight_constraint, dest=dest_lex, verbose=verbose)
    ...
    def parse(label, s, weighting=None, cascade=None, directory='', seg_units=[], verbose=False, weight_constraint=None):
        ...
        while lines:
            ...
            # State description
            m = re.match(r'descr\s+(\S+?):\s*(.*)$', line, re.U)
            if m:
            ...
            if m:
                ...
                if os.path.exists(fstpath):
                    ...
                    casc = FSTCascade.load(os.path.join(directory, casfile), seg_units=seg_units,
    ...
    def transduce(self, input, init_weight=None, split_string='', join=True, seg_units=[], reject_same=False,
                  trace=0, tracefeat='', timeit=False, timeout=0):
        ...
        for output in self.step_transduce(input, step=False, init_weight=init_weight,
            ...
            if timeout:
                if noutputs >= timeout:
                    print('Exceeded', timeout, 'outputs')
                    break
                noutputs += 1
        res = []
        for r in result:
    ...
    def _transduce_match_input(self, input, in_pos, in_string, weight=None, accum_weight=None):
        ...
        if self.is_weighted():
            return self.weighting().multiply(accum_weight, weight)
    ...
    def step_transduce(self, input, step=True, all_paths=True, init_weight=None,
        ...
        while go_on:
            ...
            while not found and go_on:
                ...
                if trace:
                    print('State:', state, 'output:', output, end=' ')
                    if in_pos >= len(input):
                        print('end of input')
                        if trace > 1: print('  weight:', accum_weight)
                    else:
                ...
                for arc in arcs:
                    ...
                    if trace:
                        print(' %s:%s' % (in_string, self.out_string(arc)), end=' ')
                        if trace > 2:
                            print(weight, end=' ')
                    # (Changed by MG.)
...
#                                print(' Matched {}'.format(tracefvals))
                ...
                if trace:
                    if any_arcs:
                        print()
                        if not any_matches:  print(' NO MATCHES')
    ...
    def composition_ep_filter(weighting=None, full=True, cascade=None):
        ...
        if cascade:
            ...
            if filt:
                return filt
        return FST.load(os.path.join(FST_DIRECTORY, label + '.fst'), weighting=weighting, cascade=cascade)
    ...
    def _composition_arc_match(self, fst2, in1, out1, in2, out2, trace=0):
        ...
        if self._label_unknown(out1):
            ...
            if trace > 1 and (stringset1 or stringset2):
                print('SS: out1', (stringset1 or out1), 'in2', (stringset2 or in2))
